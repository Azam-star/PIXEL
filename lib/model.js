...
async function callModel(req, { provider } = {}) {
  ...
  if (process.env.PIXEL_SECURE_MODE === 'true' && CLOUD_PROVIDERS.has(name)) {
    throw new Error(`[PIXEL SECURE MODE] Cloud provider "${name}" blocked! All external cloud API calls are strictly disabled in secure mode.`);
  }
