...
function sanitizeForLLM(text) {
  ...
  try {
    ...
    const injRes = scanTextForInjection(text);
    return redactText(injRes.cleanText);
  } catch {
    return text;
  }
}
...
function formatInteractive(el, view) {
  ...
  const clean = sanitizeForLLM(cleanName(el.name));
  ...
  const extras = [];
  if (el.value) {
    const cleanVal = sanitizeForLLM(cleanName(el.value));
    extras.push(`= "${truncate(cleanVal, view.maxTextChars).replace(/"/g, '\\"')}"`);
  }
