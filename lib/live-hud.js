 ...
 *
 * Features:
 * - True Jet-Black OLED Aesthetic (#000000, 40px backdrop blur, continuous squircle pill curvature)
 ...
 * - Safe DOM mounting: attaches cleanly via DOMContentLoaded even on early document creation
...
function generateLiveHudScript() {
  return `(() => {
    ...
    root.innerHTML = \`
      ...
      <div id="pixel-hud-body" style="display: flex; flex-direction: column; gap: 10px; transition: opacity 0.2s ease;">
        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 7px;">
          <button id="pixel-btn-read" class="__pixel_island_capsule__" style="padding: 8px 10px; background: rgba(255, 255, 255, 0.07); color: #f5f5f7; border: 0.5px solid rgba(255, 255, 255, 0.1); border-radius: 16px; font-weight: 600; font-size: 11px; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 6px; transition: all 0.2s ease;">
            <span style="color: #0A84FF; font-size: 12px;">👁️</span> Read Screen
          </button>
          <button id="pixel-btn-readmode" class="__pixel_island_capsule__" style="padding: 8px 10px; background: rgba(255, 255, 255, 0.07); color: #f5f5f7; border: 0.5px solid rgba(255, 255, 255, 0.1); border-radius: 16px; font-weight: 600; font-size: 11px; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 6px; transition: all 0.2s ease;">
            <span style="color: #bf5af2; font-size: 12px;">📖</span> Read Mode
          </button>
          <button id="pixel-btn-blur" class="__pixel_island_capsule__" style="padding: 8px 10px; background: rgba(255, 255, 255, 0.07); color: #f5f5f7; border: 0.5px solid rgba(255, 255, 255, 0.1); border-radius: 16px; font-weight: 600; font-size: 11px; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 6px; transition: all 0.2s ease;">
            <span style="color: #30D158; font-size: 12px;">🔒</span> Privacy Blur
          </button>
          <button id="pixel-btn-clear" class="__pixel_island_capsule__" style="padding: 8px 10px; background: rgba(255, 255, 255, 0.04); color: #86868b; border: 0.5px solid rgba(255, 255, 255, 0.08); border-radius: 16px; font-weight: 600; font-size: 11px; cursor: pointer; display: flex; align-items: center; justify-content: center; gap: 6px; transition: all 0.2s ease;">
        ...
        <div style="display: flex; gap: 7px; align-items: center; position: relative;">
          ...
          <button id="pixel-btn-run" class="__pixel_island_action_btn__" title="Run Task" style="width: 32px; height: 32px; border-radius: 50%; background: #ffffff; color: #000000; border: none; font-weight: 800; font-size: 13px; cursor: pointer; display: flex; align-items: center; justify-content: center; box-shadow: 0 2px 10px rgba(255, 255, 255, 0.35); flex-shrink: 0; transition: all 0.2s ease;">
            ↑
          </button>
        </div>
    ...
    function onDragStart(clientX, clientY) {
      ...
      currentTop = rect.top;
      root.classList.add('__pixel_island_dragging__');
    }
    ...
    function onDragMove(clientX, clientY) {
      ...
      currentLeft = Math.max(8, Math.min(window.innerWidth - root.offsetWidth - 8, initialLeft + dx));
      currentTop = Math.max(8, Math.min(window.innerHeight - root.offsetHeight - 8, initialTop + dy));
    ...
    function onDragEnd() {
      if (!isDragging) return;
      isDragging = false;
      root.classList.remove('__pixel_island_dragging__');
      if (animFrameId) {
    ...
    document.getElementById('pixel-btn-readmode').addEventListener('click', () => {
      ...
      if (isReadModeActive) {
        btn.style.background = 'rgba(191, 90, 242, 0.25)';
        btn.style.borderColor = 'rgba(191, 90, 242, 0.5)';
        btn.style.color = '#f5f5f7';
        btn.innerHTML = '<span style="color: #bf5af2; font-size: 12px;">📖</span> Read Mode [ON]';
        document.getElementById('pixel-log-msg').textContent = 'Read Mode active: non-essential peripherals dimmed.';
      } else {
        btn.style.background = 'rgba(255, 255, 255, 0.07)';
        btn.style.borderColor = 'rgba(255, 255, 255, 0.1)';
        btn.style.color = '#f5f5f7';
        btn.innerHTML = '<span style="color: #bf5af2; font-size: 12px;">📖</span> Read Mode';
    ...
    document.getElementById('pixel-btn-blur').addEventListener('click', () => {
      ...
      if (isPrivacyBlurred) {
        ...
        btn.style.color = '#f5f5f7';
        btn.innerHTML = '<span style="color: #ff453a; font-size: 12px;">🔒</span> Privacy [SHIELD]';
        document.getElementById('pixel-log-msg').textContent = 'Privacy Shield engaged: credentials & sensitive PII blurred.';
      } else {
        btn.style.background = 'rgba(255, 255, 255, 0.07)';
        btn.style.borderColor = 'rgba(255, 255, 255, 0.1)';
        btn.style.color = '#f5f5f7';
        btn.innerHTML = '<span style="color: #30D158; font-size: 12px;">🔒</span> Privacy Blur';
        document.getElementById('pixel-log-msg').textContent = 'Privacy Shield disengaged.';
      }
    });
    ...
    document.getElementById('pixel-btn-clear').addEventListener('click', () => {
      ...
      document.getElementById('pixel-btn-blur').style.background = 'rgba(255, 255, 255, 0.07)';
      document.getElementById('pixel-btn-blur').style.borderColor = 'rgba(255, 255, 255, 0.1)';
      document.getElementById('pixel-btn-blur').innerHTML = '<span style="color: #30D158; font-size: 12px;">🔒</span> Privacy Blur';
      document.getElementById('pixel-btn-readmode').style.background = 'rgba(255, 255, 255, 0.07)';
      document.getElementById('pixel-btn-readmode').style.borderColor = 'rgba(255, 255, 255, 0.1)';
      document.getElementById('pixel-btn-readmode').innerHTML = '<span style="color: #bf5af2; font-size: 12px;">📖</span> Read Mode';
      ...
      document.getElementById('pixel-privacy-status').style.color = '#30D158';
