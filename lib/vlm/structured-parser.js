...
function parseStructuredVisualOutput(rawOutput, context = {}) {
  ...
  if (!structured || typeof structured !== 'object') {
    ...
    const defaultConf = context.tier === 'B' ? 0.95 : (context.tier === 'A' ? 0.90 : 0.88);
    structured = {
  ...
  const textPart = Array.isArray(structured.text) ? structured.text.join(' ') : String(structured.text || '');
  const fullTextToScan = `${structured.summary || ''} ${structured.description || ''} ${textPart}`;
