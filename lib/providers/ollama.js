...
function parseActionsFromText(content) {
  ...
  // Pattern 3: Model outputting scroll direction, e.g. "scroll down", "scrolling down"
  const scrollMatch = text.match(/\bscroll(?:ing)?\s+(up|down|left|right)\b/i);
  if (scrollMatch) {
    return [{
      kind: 'action',
      verb: 'scroll',
  ...
  const typeMatch = text.match(/type\s+["']([^"']+)["']\s*(?:into\s*\[?(@[etrv]\d+)\]?)?/i);
  if (typeMatch) {
    return [{
      kind: 'action',
      verb: 'type',
      ref: typeMatch[2] || undefined,
      args: { text: typeMatch[1], intent: `Type ${typeMatch[1]}` },
  ...
  if (/\b(?:task is complete|task completed|task is done|finished the task)\b/i.test(text)) {
    return [{
      kind: 'action',
      verb: 'done',
