...
// PII Regex Patterns & Severity Weights (Ordered by specificity: keys/cards first)
const PII_PATTERNS = [
  {
    type: 'secret_key',
    weight: 1.0,
    regex: /\b(?:sk_live|api_key|secret_key|private_key|token)[a-zA-Z0-9_\-=]{10,}\b/gi,
  },
  {
    type: 'credit_card',
    weight: 1.0,
    regex: /\b(?:4[0-9]{12}(?:[0-9]{3})?|5[1-5][0-9]{14}|3[47][0-9]{13}|6(?:011|5[0-9]{2})[0-9]{12}|(?:[0-9]{4}[ -]){3}[0-9]{4})\b/g,
  },
  {
    type: 'aadhaar_govt_id',
    weight: 1.0,
    regex: /\b\d{4}[\s-]\d{4}[\s-]\d{4}\b|\b[A-Z]{5}\d{4}[A-Z]\b/g, // Aadhaar 12-digit formatted & PAN format
  },
  {
    type: 'pan_card',
    weight: 1.0,
  ...
  },
  {
    type: 'phone',
    weight: 0.7,
    regex: /(?:\+91[-.\s]?)?[6-9]\d{9}\b|(?:\+?1[-.\s]?)?\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4}\b/g,
  },
];
