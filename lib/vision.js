...
async function describe({ imageBase64, mimeType = 'image/png', hint, cacheKey, signal } = {}) {
  ...
  const CLOUD_PROVIDERS = new Set(['openai', 'anthropic', 'gemini', 'groq']);
  if (process.env.PIXEL_SECURE_MODE === 'true' && CLOUD_PROVIDERS.has(providerName)) {
    throw new Error(`[PIXEL SECURE MODE] Cloud vision provider "${providerName}" blocked! All external image egress is strictly disabled in secure mode.`);
  }
